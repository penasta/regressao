...
O projeto em questão visa realizar análises estatísticas abrangentes sobre o mercado imobiliário em uma localizada não especificada, com foco nos preços de venda de casas. O objetivo principal é desenvolver um modelo preditivo capaz de estimar o preço de venda de residências com base em várias características relevantes da casa e do seu entorno.

As características que serão poderão utilizadas para realizar essas predições incluem o tamanho da casa em pés quadrados, o número de quartos e banheiros, a presença de ar condicionado, o tamanho da garagem, a existência ou ausência de piscina, o ano de construção, a qualidade da construção, o tamanho do terreno e a proximidade com a rodovia. Esses atributos foram selecionados com base na suposição de que eles podem exercer influência significativa sobre os preços de venda das casas.
...
A porção inferior do retângulo diz respeito ao primeiro quartil, enquanto a superior indica o terceiro quartil. Já o traço no interior do retângulo representa a mediana do conjunto de dados, ou seja, o valor em que o conjunto de dados é dividido em dois subconjuntos de mesmo tamanho. A média é representada pelo losango branco e os pontos são \textit{outliers}. Os \textit{outliers} são valores discrepantes da série de dados, ou seja, valores que não demonstram a realidade de um conjunto de dados.
...
$$\begin{cases}
H_{0}: \mbox{A variável segue uma distribuição Normal}  \\ 
H_{1}: \mbox{A variável segue outro modelo} \\
\end{cases}$$

# Modelo de regressão linear

Modelos de regressão linear são utilizados para fazer predições de uma variável explicativa quantitativa Y com base em valores de variáveis quantitativas ou qualitativas X. O modelo se diz linear pois os parâmetros interagem entre sí de maneira linear. Um modelo de regressão linear múltiplo generalizado é dado pela expressão abaixo:

$$Y_i=\beta_0 + \beta_1 X_{1i} + \beta_2 X_{2i} + \ldots + \beta_k X_{ki} +\epsilon_i$$

Onde: $Y_i$ é o valor da variável resposta para uma dada observação; $\beta_0$ é o valor esperado de Y caso os valores de todas as variáveis explicativas sejam 0, também chamado de intercepto; $\beta_k$ é o aumento na variável resposta causado pelo aumento de uma unidade da variável $X_k$; $X_{ki}$ é o valor observado da k-ésima variável explicativa na i-ésima observação, essas variáveis podendo ser transformações de outras variáveis, contanto que $X_k$ sejam independentes; $\epsilon_i$ é o componente do erro aleatório.

Para o modelo de regressão linear, se assume que $\epsilon_i$ são independentes e identicamente distribuidos, seguindo distribuição normal com média 0 e variância constante $\sigma^2$.

A partir do modelo acima, um modelo pode ser ajustado para fazer predições da variável explicativa, dado pela expressão abaixo:
...
Onde: $\textbf{Y}$ é o vetor das observações da variável resposta; $\textbf{X}$ é a matriz das observações das variáveis explicativas; $\boldsymbol\beta$ é o vetor dos parâmetros; $\boldsymbol\epsilon$ é o vetor dos erros das observações.

O modelo para predições em forma matricial é dado por:
$$\hat{\textbf{Y}}=\textbf{X}\hat{\boldsymbol\beta}$$

Onde: $\hat{\textbf{Y}}$ é o vetor das predições da variável resposta; $\textbf{X}$ é a matriz das observações das variáveis explicativas; $\hat{\boldsymbol\beta}$ é o vetor dos estimadores dos parâmetros.
...
$$ \hat{\boldsymbol\beta}= \left( \textbf{X}'\textbf{X} \right) ^{-1} \textbf{X}' \textbf{Y}$$

Sendo:
\begin{itemize}
\item $\textbf{Y} = $ vetor de observações da variável resposta 
\item $\textbf{X}=$ matriz de observações do conjunto de variáveis explicativas
\end{itemize}
...
\item $\hat{y}_i=$ predição da variável $Y$ para a i-ésima observação
\end{itemize}
...
\begin{itemize}
\item $n = $ tamanho da amostra
\item $e_i=$ resídou para a i-ésimo observação
\end{itemize}

## Soma de Quadrados da Regressão (SSReg)

Mede a variação total explicada pelo conjunto de variáveis \textbf{X}. O seu calculo é dado pela seguinte formula:

$$ SSReg = \sum_{i=1}^{n} \left(\hat{y_i} - \bar{y}\right)^{2}  $$
...
É uma estimativa não viesada da variância $\sigma^2$ dos erros $\epsilon_i$ nos modelos de regressão linear. É um componente utilizado no cálculo de várias estatísticas para testes relacionados à regressão. Seu calculo é dado pela seguinte formula:
...
\begin{itemize}
\item $n = $ tamanho da amostra
\item $p=$ número de parâmetros no modelo
\item $SSE=$ soma de quadrados dos resíduos
\end{itemize}
...
\begin{itemize}
\item $p=$ número de parâmetros no modelo
\item $SSReg=$ soma de quadrados da regressão
\end{itemize}
...
$$t_i = e_i\left[ \frac{n-p-1}{\sqrt{SSE\left( 1-h_{ii} \right)-e_i^2}} \right]^{1/2} $$
...
\item $p=$ número de parâmetros no modelo
\item $SSE = $ soma de quadrados dos resíduos
\item $h_{ii} =$ alavanca para a i-ésimo observação
\end{itemize}

## DFFITS

Mede a influência que uma observação tem na predição de seu própio valor. Serve para avaliar se uma observação é influente no calculo de seu valor predito. Seu calculo é dado pela formula:

$$DFFITS_i = t_i\left( \frac{h_{ii}}{1-h_{ii}} \right)^{1/2}$$

Sendo:
\begin{itemize}
\item $t_i=$ resíduo studentizado externamente para a i-ésimo observação
\item $h_{ii} =$ alavanca para a i-ésimo observação
\end{itemize}
...
$$D_i = \frac{e_i^2}{p MSE}\left( \frac{h_{ii}}{\left( 1-h_{ii} \right)^2} \right)$$

Sendo:
\begin{itemize}
\item $e_i=$ resídou para a i-ésimo observação
\item $p=$ número de parâmetros no modelo
\item $MSE=$ quadrado médio dos resíduos
\item $h_{ii} =$ alavanca para a i-ésimo observação
\end{itemize}
...
$$\left(DFBETAS_{k}\right)_{i} = \frac{\hat{\beta}_k-\hat{\beta}_{k(i)}}{\sqrt{MSE_{(i)}c_{kk}}}$$

Sendo:
\begin{itemize}
\item $\hat{\beta}_k=$ estimativa do k-ésimo parâmetro
\item $\hat{\beta}_{k(i)}=$ estimativa do k-ésimo parâmetro sem a i-ésima observação
\item $MSE_{(i)}=$ quadrado médio dos resíduos sem a i-ésima observação
\item $c_{kk} =$ k-ésimo valor na diagonal da matriz $\left(\textbf{X}'\textbf{X}\right)^{-1}$, onde:
\begin{itemize}
\item $\textbf{X}=$ matriz de observações do conjunto de variáveis explicativas
\end{itemize} 
\end{itemize}
...
Caclula quanto que a informação de uma variável ou um conjunto de variáveis explicativas \textbf{X} reduz a variância de uma variável resposta Y. É utilizado para medir quão bem as variáveis explicativas \textbf{X} explicam a variável resposta Y. Procura-se um modelo com um alto coeficiente de determinação. Em estudos de regressão, seu caclulo é dado pela seguinte expressão:
...
\begin{itemize}
\item $SSE = $ soma de quadrados dos resíduos
\item $SSTO=$ soma de quadrados total
\end{itemize} 

## Coeficiente de Determinação Ajustado ($R_{a}^2$)

Estimativa para o grau de explicação do modelo. Similar ao coeficiente de determinação, mas penalizado pela inclusão de novas variáveis no modelo. Procura-se um modelo com um alto coeficiente de determinação ajustado. Seu caclulo é dado pela seguinte expressão:
 
$$ R_{a}^2 = 1 - \frac{n-1}{n-p} \frac{SSE}{SSTO} $$
...
\begin{itemize}
\item $n = $ tamanho da amostra
\item $p=$ número de parâmetros no modelo testado
\item $SSE_p = $ soma de quadrados dos resíduos do modelo com p variáveis
\item $MSE=$ quadrado médio dos resíduos do modelo com todas variáveis
\end{itemize} 

## Métodos de seleção automática

São algoritmos iterativos que buscam o melhor modelo com base em uma serie de testes F de ausência de regressão e testes T de ausência de um parâmetro. Os métodos visam um modelo em que todas as variáveis inclusas foram significantes, ou todas as retiradas foram não significantes em ao menos uma iteração. Para todos os métodos, não há garantia que o melhor modelo será encontrado.
...
$$\begin{cases}
H_{0}: \mbox{$\beta_k=0, k=1, 2, ..., p-1$} \\ 
H_{1}: \mbox{$\exists \beta_k \neq 0, k = 1, 2, ..., p-1$} \\
\end{cases}$$
...
Sendo:
\begin{itemize}
\item $MSReg=$ quadrado médio da regressão
\item $MSE=$ quadrado médio dos resíduos
\end{itemize}
...
\begin{itemize}
\item $\hat{\beta}_k=$ estimativa do k-ésimo parâmetro
\item $S(\hat{\beta}_k)=$ raiz quadrada da variação da estimativa do k-ésimo parâmetro
\end{itemize}
...
É importante destacar as limitações e vantagens do modelo. As principais vantagens foi o fato de dispormos de uma gama de variáveis quantitativas que facilita a execução do modelo. Além disso, tivemos acesso aos dados já estruturados, o que dispensa uma etapa importante de ETL.
Quanto as limitações, não sabemos exatamente a localidade da coleta dos dados, portanto, tampouco podemos fazer muita inferência sobre o uso deste modelo para predições em geral. Além disso, pelas limitações do modelo regressivo múltiplo, tivemos por vezes que descartar informações que num modelo mais sofisticado, poderiam ser úteis para a inferência dos resultados e sua maior precisão. 